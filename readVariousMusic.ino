...
long endTimeOfThePreviousEngine; //variable that stores the end time of the previous engine
...
String input; //variable responsible for receiving the name of the song in the serial input.
...
void loop(){
  ...
  if(Serial.available()){
    ...
    input += ".txt"; //concatenate with the file extension
    
    const char* conv = input.c_str(); //Variable responsible to show what will be the command to be executed
